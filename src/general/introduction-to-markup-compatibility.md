...
When you open a document using the Open XML SDK, you have the option of using an overload with a signature that accepts an instance of the `DocumentFormat.OpenXml.Packaging.OpenSettings` class as a parameter. You use the open settings class to provide certain important settings that govern the behavior of the SDK. One set of settings in particular, stored in the `DocumentFormat.OpenXml.Packaging.OpenSettings.MarkupCompatibilityProcessSettings` property, determines how markup compatibility elements and attributes are processed. You set the property to an instance of the `DocumentFormat.OpenXml.Packaging.MarkupCompatibilityProcessSettings` class prior to opening a document.

The class has the following properties:

- `DocumentFormat.OpenXml.Packaging.MarkupCompatibilityProcessSettings.ProcessMode` - Determines the parts that are preprocessed.

- `DocumentFormat.OpenXml.Packaging.MarkupCompatibilityProcessSettings.TargetFileFormatVersions` - Specifies the context that applies to preprocessing.
...
The target file format versions property lets you choose to process markup compatibility content in the context of a specific Office version, e.g. Office 365. Set the `TargetFileFormatVersions` property to a member of the `DocumentFormat.OpenXml.FileFormatVersions` enumeration.
