...
file name by using the `DocumentFormat.OpenXml.Packaging.WordprocessingDocument.Open%2A` method, with the Boolean parameter.
...
documents as `DocumentFormat.OpenXml.Packaging.WordprocessingDocument`
objects, and creates variables that reference the `DocumentFormat.OpenXml.Packaging.ThemePart` parts in each of the packages.
...
The code then reads the contents of the source `DocumentFormat.OpenXml.Packaging.ThemePart`  part by using a `StreamReader` object and writes to the target
`DocumentFormat.OpenXml.Packaging.ThemePart` part by using a `System.IO.StreamWriter`.
