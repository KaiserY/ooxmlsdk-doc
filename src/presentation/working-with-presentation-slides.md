...
This topic discusses the Open XML SDK for Office `DocumentFormat.OpenXml.Presentation.Slide` class and how it relates to the Open
...
`<transition/>` element. The following information from the [ISO/IEC 29500](https://www.iso.org/standard/71691.html)
...
instance of the `DocumentFormat.OpenXml.Packaging.SlidePart` class, which represents a slide
part, one of the required parts of a PresentationML presentation file
package. Each instance of the `Slide` class
must also be associated with instances of the `DocumentFormat.OpenXml.Presentation.SlideLayout` and `DocumentFormat.OpenXml.Presentation.SlideMaster` classes, which are in turn
associated with similarly named required presentation parts, represented
by the `DocumentFormat.OpenXml.Packaging.SlideLayoutPart` and `DocumentFormat.OpenXml.Packaging.SlideMasterPart` classes.
...
these classes, as shown in the following code example, are the `CommonSlideData` class, the `ColorMapOverride` class, the `DocumentFormat.OpenXml.Presentation.ShapeTree` class, and the `DocumentFormat.OpenXml.Presentation.Shape` class.
...
the `DocumentFormat.OpenXml.Presentation.NonVisualGroupShapeProperties` class, the `DocumentFormat.OpenXml.Presentation.GroupShapeProperties` class, and the `Shape` class.
