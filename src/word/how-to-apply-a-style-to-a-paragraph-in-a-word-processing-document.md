...
represented by the `DocumentFormat.OpenXml.Wordprocessing.ParagraphProperties` class. The code
determines if the element exists, and creates a new instance of the
`ParagraphProperties` class if it does not.
The `pPr` element is a child of the `p` (paragraph) element; consequently, the `DocumentFormat.OpenXml.OpenXmlElement.PrependChild` method is used to add
...
element is represented by the `DocumentFormat.OpenXml.Wordprocessing.Styles` class in the Open XML SDK. Finally,
...
`DocumentFormat.OpenXml.Packaging.MainDocumentPart.StyleDefinitionsPart` of the main document
...
The `System.Linq.Enumerable.FirstOrDefault`
...
such as the `DocumentFormat.OpenXml.Wordprocessing.Style.Type` of style (paragraph) and
`DocumentFormat.OpenXml.Wordprocessing.Style.StyleId`. As mentioned above, the styleid is
...
`DocumentFormat.OpenXml.Wordprocessing.StyleRunProperties` class and then appends
