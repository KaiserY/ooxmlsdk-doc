...
It is not enough to delete the part from the document. You must also convert the document type, internally. The Open XML SDK provides a way to perform this task: You can call the document `ChangeDocumentType` method and indicate the new document type (in this case, supply the `DocumentFormat.OpenXml.WordprocessingDocumentType.Document?displayProperty=nameWithType` enumerated value).
