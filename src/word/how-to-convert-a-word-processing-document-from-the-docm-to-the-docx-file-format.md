...
In `ooxmlsdk`, `MainDocumentPart::vba_project_part(&document)` can locate a VBA project part when one is related from the main document part. A full converter also needs package mutation support for deleting that part, removing its relationship, changing content types, and saving to the new `.docx` path.
